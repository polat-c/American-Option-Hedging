...
def compute_stoppings(S0, K, T, N, sigma, r=0):
    ...
    for k in range(N - 1, -1, -1):  # iterate through timesteps
        for m in range(0, k + 1):  # iterate through possible up-down movements
            ...
            C[(k, m)] = max(math.exp(-r * dt) * edv,
                            intrinsic_value)  # since american option can be exercised before maturity

            if intrinsic_value > edv:
                stopping_nodes.append([k,m])
